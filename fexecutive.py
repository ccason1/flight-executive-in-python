...
class TelemetryPoint:
    ...
    def __init__(self, name, min_time_interval, data_getter, header, filename=None):
        ...
        if filename == None:
            self.filename = self.output_file_path + '/' + name + '_log.csv'
        else:
...
def setup_telemetry_points():
    ...
    acc = TelemetryPoint(name='acc', min_time_interval=2, data_getter=get_acc_data, header='acc')
    ...
    tmp = TelemetryPoint(name='tmp', min_time_interval=5, data_getter=get_tmp_data, header='tmp')
    ...
    env = TelemetryPoint(name='env', min_time_interval=8, data_getter=get_env_data, header='env')
